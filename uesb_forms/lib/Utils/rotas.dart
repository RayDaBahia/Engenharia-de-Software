class Rotas {
  static const HOME = '/';

  static const MEUS_FORMULARIOS = '/Meus-Formularios';

  static const CRUD_BANCO = '/Crud-Bancos';

  static const MEUS_BANCOS = '/Meus-Bancos';

  static const SELECIONAR_QUESTOES_FORMULARIO =
      '/Selecionar-Questoes-Formulario';

  static const SELECAO_QUESTOES_BANCO = '/Selecionar-Questoes-Banco';

  static const EDICAO_FORMULARIO_TELA = '/EdicaoBanco';

  static const CONFIGURAR_ACESSO_FORMS = '/Configurar-Acesso-Formulario';


  static const MEUS_GRUPOS='/meusGrupos';

  static const CRIAR_GRUPO= '/criarGrupo';

  static const GRUPO= '/Grupo';


}
